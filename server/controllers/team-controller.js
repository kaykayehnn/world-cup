...
exports.getTeamMatches = (req, res, next) => {
  getTeamMatches(req.params.teamName)(req.cache).then(data => {
    if (data.length === 0) {
...
exports.getMatchById = (req, res, next) => {
  getMatch(+req.params.matchId)(req.cache).then(data => {
    if (!data) {
